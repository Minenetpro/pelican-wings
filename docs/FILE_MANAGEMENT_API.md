...
7. [Direct File Transfer](#direct-file-transfer)
   - [Download File](#get-downloadfile)
   - [Download Backup](#get-downloadbackup)
...
These endpoints use signed tokens for authentication instead of Bearer tokens.
...
**Note:** The signed token is typically generated by the Panel and includes the server UUID, file path, and expiration time.
...
Content-Disposition: attachment; filename="backup-uuid.tar.gz"
Content-Type: application/octet-stream
```
...
**Limits:**
- `api.upload_limit` (default 100 MiB per file)
...
```bash
curl -X POST "https://wings.example.com/upload/file?token=<signed-token>&directory=/" \
  -F "files=@local-file.jar"
```
